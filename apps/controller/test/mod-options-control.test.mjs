...
function createViewerAuthToken({ secret, sessionId, userId, userName, tenantSlug = 'default' }) {
  const now = Date.now();
  const payload = {
    v: 1,
    type: 'viewer-auth',
...
test('mod state includes active player identities and win flags', { timeout: 90000 }, async () => {
  const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), 'bingo-controller-mod-state-active-players-it-'));
  const stateFilePath = path.join(tempDir, 'controller-state.json');
  const viewerAuthSecret = 'controller-test-viewer-auth-secret';
  let runtime = null;

  try {
    const port = await getFreePort();
    runtime = startController(port, stateFilePath, {
      VIEWER_AUTH_TOKEN_SECRET: viewerAuthSecret,
    });
    await waitForHealth(runtime.baseUrl, runtime.child, runtime.logs);
    ...
    assert.equal(startSessionResponse.status, 200, JSON.stringify(startSessionResponse.json));
    const sessionId = startSessionResponse.json?.session?.id;
    assert.equal(typeof sessionId, 'string');

    const stateBeforeJoin = await requestJson(runtime.baseUrl, 'GET', '/api/mod/state', undefined, modHeaders());
    assert.equal(stateBeforeJoin.status, 200, JSON.stringify(stateBeforeJoin.json));
    assert.ok(Array.isArray(stateBeforeJoin.json?.activePlayers));
    assert.equal(stateBeforeJoin.json.activePlayers.length, 0);

    const viewerAuthToken = createViewerAuthToken({
      secret: viewerAuthSecret,
      sessionId,
      userId: 'viewer-123',
      userName: 'Viewer One',
    });
    ...
    assert.equal(joinResponse.json?.ok, true);

    const stateAfterJoin = await requestJson(runtime.baseUrl, 'GET', '/api/mod/state', undefined, modHeaders());
    assert.equal(stateAfterJoin.status, 200, JSON.stringify(stateAfterJoin.json));
    assert.ok(Array.isArray(stateAfterJoin.json?.activePlayers));

    const joinedPlayer = stateAfterJoin.json.activePlayers.find((entry) => entry.userId === 'viewer-123');
    assert.ok(joinedPlayer, JSON.stringify(stateAfterJoin.json));
    assert.equal(joinedPlayer.userName, 'Viewer One');
    assert.equal(joinedPlayer.hasWon, false);
    assert.deepEqual(Object.keys(joinedPlayer).sort(), ['hasWon', 'userId', 'userName']);
    ...
    const stateAfterEnd = await requestJson(runtime.baseUrl, 'GET', '/api/mod/state', undefined, modHeaders());
    assert.equal(stateAfterEnd.status, 200, JSON.stringify(stateAfterEnd.json));
    assert.ok(Array.isArray(stateAfterEnd.json?.activePlayers));
    assert.equal(stateAfterEnd.json.activePlayers.length, 0);
  } finally {
    await stopController(runtime?.child);
    await fsPromises.rm(tempDir, { recursive: true, force: true });
  }
...
test('mod theme endpoints require auth and apply palette updates', { timeout: 90000 }, async () => {
  ...
  try {
    ...
    assert.equal(idleSession.json?.theme?.skinId, undefined);
    ...
    assert.equal(updateTheme.json?.theme?.skinId, 'candlelit-pumpkins');
    assert.equal(updateTheme.json?.controllerTheme?.skinId, 'candlelit-pumpkins');
    assert.equal(updateTheme.json?.cardsTheme?.skinId, 'candlelit-pumpkins');
    ...
    assert.equal(themeAfterUpdate.json?.theme?.skinId, 'candlelit-pumpkins');

    const sessionAfterSkinSet = await requestJson(runtime.baseUrl, 'GET', '/api/session');
    assert.equal(sessionAfterSkinSet.status, 200, JSON.stringify(sessionAfterSkinSet.json));
    assert.equal(sessionAfterSkinSet.json?.theme?.skinId, 'candlelit-pumpkins');
    ...
    assert.equal(modStateAfterScopes.json?.theme?.skinId, 'candlelit-pumpkins');
    assert.equal(modStateAfterScopes.json?.controllerTheme?.skinId, 'candlelit-pumpkins');
    assert.equal(modStateAfterScopes.json?.cardsTheme?.skinId, 'candlelit-pumpkins');
