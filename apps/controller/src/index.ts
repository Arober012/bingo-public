...
const themeSkinIdPattern = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
...
const persistedThemeStateSchema = z.object({
  ...
  skinId: z.union([z.string().regex(themeSkinIdPattern), z.literal(''), z.null()]).optional(),
  updatedAt: z.string(),
});
...
const modTestingAllowlist = new Set(
  (process.env.MOD_TESTING_ALLOWLIST ?? '')
    .split(',')
...
const modThemeSkinIdPatchSchema = z.union([z.string().regex(themeSkinIdPattern), z.literal(''), z.null()]);
...
const modThemeUpdateSchema = z
  .object({
    ...
    skinId: modThemeSkinIdPatchSchema.optional(),
  })
  .refine(
    (value) =>
    ...
    { message: 'Provide mode, palette, resetOverrides, linkControllerAndCards, or skinId.' },
  );
...
function normalizeThemeSkinId(input: string | null | undefined): string | undefined {
  const normalized = String(input ?? '').trim().toLowerCase();
  if (!normalized) {
    return undefined;
  }

  return themeSkinIdPattern.test(normalized) ? normalized : undefined;
}
...
function resolveThemeState(state: ThemeState, surface: 'controller' | 'cards' = 'cards'): ResolvedTheme {
  ...
  return {
    ...
    skinId: normalizeThemeSkinId(state.skinId),
...
function applyThemePatch(state: ThemeState, patch: z.infer<typeof modThemeUpdateSchema>): ThemeState | null {
  ...
  let nextSkinId = normalizeThemeSkinId(state.skinId);
  ...
  if (patch.skinId !== undefined) {
    const normalizedPatchSkinId = normalizeThemeSkinId(patch.skinId);
    if (normalizedPatchSkinId !== nextSkinId) {
      nextSkinId = normalizedPatchSkinId;
      changed = true;
    }
  }
...
function isModTestingIdentityAllowlisted(userId: string | null, userLogin: string | null): boolean {
  if (modTestingAllowlist.size === 0) {
    return false;
  }

  const normalizedUserId = String(userId ?? '').trim().toLowerCase();
  if (normalizedUserId && modTestingAllowlist.has(normalizedUserId)) {
    return true;
  }

  const normalizedUserLogin = String(userLogin ?? '').trim().toLowerCase();
  if (normalizedUserLogin && modTestingAllowlist.has(normalizedUserLogin)) {
    return true;
  }
...
function toActivePlayerList(session: SessionState | null): ActivePlayerSummary[] {
  if (!session || (session.status !== 'open' && session.status !== 'running')) {
    return [];
  }
  ...
  try {
    ...
    const payload: PersistedControllerState = {
      ...
      themeState: {
        ...
        skinId: normalizeThemeSkinId(tenantState.themeState.skinId),
  ...
  try {
    ...
    tenantState.themeState = persistedState.themeState
      ? {
          ...
          skinId: normalizeThemeSkinId(persistedState.themeState.skinId),
...
app.post('/api/mod/testing/viewer-link', (req, res) => {
  ...
  const tenantContext = getTenantContext(req);
  const tenantState = getTenantRuntimeState(tenantContext.slug);
  const modAccess = resolveModAccess(req);
  ...
  if (
    modAccess.context.legacyTokenUsed
    || !modAccess.context.userId
    || !modAccess.context.userLogin
    || !modAccess.context.userName
  ) {
    res.status(403).json({
      error: 'testing_identity_required',
      authenticated: true,
      authorized: true,
      legacyTokenUsed: modAccess.context.legacyTokenUsed,
      message: 'Testing viewer links require Twitch session auth with a Twitch user identity.',
    });
    return;
  }

  if (!isModTestingIdentityAllowlisted(modAccess.context.userId, modAccess.context.userLogin)) {
    res.status(403).json({
      error: 'testing_access_denied',
      authenticated: true,
      authorized: true,
      message: 'This Twitch account is not allowlisted for testing viewer-link access.',
      user: {
        userId: modAccess.context.userId,
        userLogin: modAccess.context.userLogin,
        userName: modAccess.context.userName,
      },
    });
    return;
  }

  if (!viewerBaseUrl || !viewerAuthTokenSecret) {
    res.status(503).json({
      error: 'testing_viewer_link_not_configured',
      message: 'Set VIEWER_BASE_URL and VIEWER_AUTH_TOKEN_SECRET to enable testing viewer links.',
    });
    return;
  }
  ...
  if (!activeSession || (activeSession.status !== 'open' && activeSession.status !== 'running')) {
    res.status(400).json({
      ...
      message: 'Bingo join is not available right now.',
    });
    return;
  }

  const userId = modAccess.context.userId;
  const userName = sanitizeUserName(modAccess.context.userName, modAccess.context.userLogin);
  const existedBeforeJoin = activeSession.players.has(userId);

  const joinResult = joinSession(userId, userName, tenantContext.slug, tenantState);
  const nextSession = tenantState.activeSession;
  if (!joinResult.ok || !nextSession) {
    res.status(400).json({
      error: 'join_not_available',
  ...
  if (!player) {
    res.status(500).json({
      error: 'testing_player_not_found',
      message: 'Failed to load a testing player card for this moderator.',
    });
    return;
  }
  ...
  const authPayload = parseViewerAuthToken(authToken);
  if (!authPayload) {
    res.status(500).json({
      error: 'testing_auth_token_generation_failed',
      message: 'Viewer auth token generation failed for testing card link.',
    });
    return;
  }
  ...
  commitControllerState(
    'mod_testing_viewer_link_generated',
    `Testing viewer card link generated for ${player.userName}.`,
  ...
  res.json({
    ...
    userId: player.userId,
    userName: player.userName,
    joined: !existedBeforeJoin,
    expiresInSeconds: Math.max(0, Math.floor((authPayload.exp - Date.now()) / 1000)),
  });
});
...
app.get('/api/mod/state', (req, res) => {
  ...
  res.json({
    ...
    activePlayers: toActivePlayerList(activeSession),
